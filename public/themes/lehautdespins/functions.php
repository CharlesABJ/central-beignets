...
function sq_insert_css_in_head()
{
  ...
  if (
    ...
    wp_enqueue_style('lehautdespins/', get_theme_file_uri('assets/' . $manifest['resources/js/index.js']['css'][0]));
  }
}
...
function sq_insert_js_in_footer()
{
  ...
  if (
    ...
    wp_enqueue_script('lehautdespins/', 'http://localhost:5173/resources/js/index.js');
  } elseif (file_exists($manifestPath)) {
    $manifest = json_decode(file_get_contents($manifestPath), true);
    wp_enqueue_script('lehautdespins/', get_theme_file_uri('assets/' . $manifest['resources/js/index.js']['file']));
  }
}
...
add_filter('script_loader_tag', function (string $tag, string $handle, string $src) {
  if (in_array($handle, ['vite', 'lehautdespins/'])) {
