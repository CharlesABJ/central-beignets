...
function sq_insert_css_in_head()
{
  ...
  if (
    ...
    wp_enqueue_style('pandoracase', get_theme_file_uri('assets/' . $manifest['resources/js/index.js']['css'][0]));
  }
}
...
function sq_insert_js_in_footer()
{
  ...
  if (
    ...
    wp_enqueue_script('pandoracase', 'http://localhost:5173/resources/js/index.js');
  } elseif (file_exists($manifestPath)) {
    $manifest = json_decode(file_get_contents($manifestPath), true);
    wp_enqueue_script('pandoracase', get_theme_file_uri('assets/' . $manifest['resources/js/index.js']['file']));
  }
}
